...
export const RecordAnswer = ({
  ...
  const recordUserAnswer = async () => {
    if (isRecording) {
      ...
      if (userAnswer?.length < 30) {
        toast.error("Error", {
          description:
            "Please provide a longer answer with more than 30 characters.",
        });
        return;
      }
  ...
  const saveUserAnswer = async () => {
   ...
   try {
    //query the firebase to check if the user anser already exists for this question
    const userAnswerQuery = query(collection(db,"userAnswers"),where("userId","==",userId),where("question","==",currentQuestion ));

    const querySnap = await getDocs(userAnswerQuery);

    //if user has already answered this question, we won't save it again
    if(!querySnap.empty){
      ...
      toast.success("Saved", {description: "Your answer has been saved successfully."});
    }
    setUserAnswer("");
    stopSpeechToText();
    setOpen(!open);
    ...
    <div className="w-full flex flex-col items-center gap-8 mt-4">
      ...
      <div className="w-full mt-4 p-4 border rounded-md bg-gray-50">
        ...
        <p className="text-sm mt-2 text-gray-700 whitespace-normal">
          {userAnswer ||
            "No answer recorded yet.Start recording to answer the question."}
        </p>
